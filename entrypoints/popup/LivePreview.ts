import { type EditorSelection, type EditorState, Facet, type Range } from "@codemirror/state";
...
import { MediaWidget, mediaChoices, mediaKindOf, redrawPreview, safeMediaUrl } from "./LiveMedia";
...
/** Whether live preview loads web media without asking. Off by default for privacy. */
  ...
  for (const visibleRange of view.visibleRanges) {
    tree.iterate({
      ...
      enter(node) {
        ...
        // A plugin may not replace a line break, so embeds written across lines stay as text.
        const { doc } = view.state;
        if (node.name === "Image" && doc.lineAt(node.from).number === doc.lineAt(node.to).number) {
          const url = node.node.getChild("URL");
          const src = url ? safeMediaUrl(view.state.sliceDoc(url.from, url.to)) : null;
          const altEnd = node.node.getChildren("LinkMark")[1]?.from ?? node.from;
          if (src) {
            const alt = view.state.sliceDoc(node.from + 2, altEnd);
            // `![alt](url)` embeds whatever the URL is, and is an image unless it says otherwise.
            const kind = mediaKindOf(src) ?? "image";
            const shown = mediaChoices.get(src) ?? (!src.startsWith("http") || view.state.facet(loadRemoteImages));
            if (!selectionTouches(view, node.from, node.to)) {
              ranges.push(
                Decoration.replace({ widget: new MediaWidget(src, alt, kind, shown, "inline") }).range(node.from, node.to),
              );
              return false;
            }
            // While the cursor is on its Markdown, loaded media stays put above the source
            // rather than collapsing, so the text around it doesn't jump under the pointer.
            if (shown) {
              ranges.push(
                Decoration.widget({ widget: new MediaWidget(src, alt, kind, shown, "above"), side: -1 }).range(node.from),
              );
            }
          }
        }

        // A link to a media file stays a link, with an icon after it that shows the file below
        // the line, so a sentence isn't split around it.
        const parentName = node.node.parent?.name;
        if (node.name === "Link" || (node.name === "URL" && parentName !== "Link" && parentName !== "Image")) {
          const url = node.name === "Link" ? node.node.getChild("URL") : node.node;
          const src = url ? safeMediaUrl(view.state.sliceDoc(url.from, url.to)) : null;
          const kind = src?.startsWith("http") ? mediaKindOf(src) : null;
          if (src && kind) {
            const marks = node.node.getChildren("LinkMark");
            const text = node.name === "Link" && marks[1] ? view.state.sliceDoc(node.from + 1, marks[1].from) : "";
            // `<url>` ends after its closing bracket.
            const end = parentName === "Autolink" ? (node.node.parent?.to ?? node.to) : node.to;
            const shown = mediaChoices.get(src) ?? false;
            ranges.push(
              Decoration.widget({ widget: new MediaWidget(src, text, kind, shown, "after"), side: 1 }).range(end),
            );
            if (shown) {
              ranges.push(
                Decoration.widget({ widget: new MediaWidget(src, text, kind, shown, "below"), side: 2 }).range(doc.lineAt(end).to),
              );
            }
          }
        }
